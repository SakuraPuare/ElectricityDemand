...
def analyze_demand_y_distribution(sdf_demand: DataFrame, sample_frac=0.005, random_state=42, plots_dir=None):
    ...
    try:
        ...
        non_positive_perc = (non_positive_count / (total_count - null_count)) * 100 if (total_count - null_count) > 0 else 0
        logger.info(f"全量数据中 'y' <= 0 的数量 (在非缺失值中): {non_positive_count:,} ({non_positive_perc:.2f}%)")
        ...
        if plots_dir:
            ...
            try:
                ...
                if y_sample_pd is None or y_sample_pd.empty:
                    ...
                    # --- 4. 调用绘图函数 ---
                    plot_demand_y_distribution(y_sample_pd, plots_dir, random_state=random_state)
...
def plot_demand_y_distribution(y_sample_pd, plots_dir, plot_sample_size=100000, random_state=42):
    ...
    try:
        ...
        y_plot_sample_non_neg = pd.to_numeric(y_plot_sample_non_neg, errors='coerce').dropna()
        if y_plot_sample_non_neg.empty:
            logger.warning("No numeric non-negative values after coercion, skipping log plot.")
            return

        y_log_transformed = np.log1p(y_plot_sample_non_neg + epsilon) # 使用 np.log1p
...
def analyze_demand_timeseries_sample(sdf_demand: DataFrame, n_samples=5, plots_dir=None, random_state=42):
    ...
    if not all(col in sdf_demand.columns for col in required_cols):
         logger.error(f"Input Spark DataFrame missing required columns ({', '.join(required_cols)}).")
         return
    ...
    try:
        ...
        if n_samples <= 0:
             logger.warning(f"Requested sample size ({n_samples}) is not positive, skipping.")
             return

        actual_n_samples = min(n_samples, num_distinct_ids)
        if num_distinct_ids < n_samples:
            logger.warning(f"Total unique_id count ({num_distinct_ids}) is less than requested sample size ({n_samples}), using all {num_distinct_ids} unique_ids.")

        logger.info(f"Randomly sampling {actual_n_samples} unique_ids from {num_distinct_ids:,} using Spark RDD takeSample...")
        # takeSample returns a list of Row objects
        sampled_ids_rows = all_unique_ids_sdf.rdd.takeSample(False, actual_n_samples, seed=random_state)
        ...
        sampled_ids_sdf = sdf_demand.sql_ctx.createDataFrame([(id,) for id in sampled_ids], ['unique_id'])
        # 使用 broadcast join 提示，因为 sampled_ids_sdf 很小
        sdf_sample_filtered = sdf_demand.join(F.broadcast(sampled_ids_sdf), on='unique_id', how='inner')
        ...
        try:
            ...
            pdf_sample = sdf_sample_filtered.select(required_cols).toPandas() # 只选择需要的列
            if pdf_sample is None or pdf_sample.empty:
                logger.error("收集样本数据到 Pandas 失败或结果为空。")
                return
            logger.info(f"Pandas DataFrame created from Spark sample, containing {len(pdf_sample):,} rows for {len(sampled_ids)} unique_ids.")
        except Exception as collect_e:
             logger.exception(f"收集 Spark 样本数据到 Pandas 时出错: {collect_e}. 尝试禁用 Arrow...")
             try:
                  # 尝试禁用 Arrow 并重试
                  sdf_demand.sql_ctx.setConf("spark.sql.execution.arrow.pyspark.enabled", "false")
                  ...
                  if pdf_sample is None or pdf_sample.empty:
                       ...
                       return
                  logger.info(f"(Arrow 已禁用) Pandas DataFrame created, containing {len(pdf_sample):,} rows.")
             except Exception as collect_e_no_arrow:
                  logger.exception(f"禁用 Arrow 后收集 Spark 样本数据到 Pandas 时仍然出错: {collect_e_no_arrow}")
                  return
             finally:
                   # 恢复 Arrow 设置 (可选)
                   sdf_demand.sql_ctx.setConf("spark.sql.execution.arrow.pyspark.enabled", "true")
        ...
        if not pd.api.types.is_datetime64_any_dtype(pdf_sample['timestamp']):
             ...
             if pdf_sample['timestamp'].isnull().any():
                 logger.warning("Some timestamp values failed to convert to datetime in Pandas.")
                 pdf_sample = pdf_sample.dropna(subset=['timestamp']) # 移除转换失败的行
        ...
        # for unique_id in tqdm(sampled_ids, desc="Processing samples (Pandas)"): # 旧方法
            ...
            try:
                ...
                ax.plot(df_id['timestamp'], df_id['y'], label=f'ID: {unique_id}', marker='.', linestyle='-', markersize=2) # 小点+线
            ...
            try:
                 # 确保已排序 (已在分组前排序)
                 time_diffs = df_id['timestamp'].diff().dropna() # 计算差值并移除第一个 NaN
                 if not time_diffs.empty:
                     ...
                     if not freq_counts.empty:
                         ...
                         logger.info(f" unique_id '{unique_id}' has only one unique time interval after diff/dropna in sample.")
                 else:
                     logger.info(f" unique_id '{unique_id}' has less than two timestamps in sample, cannot calculate intervals.")
