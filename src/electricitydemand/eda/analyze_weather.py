...
def analyze_weather_numerical(sdf_weather: DataFrame, columns_to_analyze=None, plot_sample_frac=0.05, plots_dir=None, random_state=42): # Changed sample frac default
    ...
    if columns_to_analyze is None:
        ...
        columns_to_analyze = [
            ...
            'soil_tepmerature_0_to_7cm', 'soil_moisture_0_to_7cm', # Typo in original name 'tepmerature' kept if that's the column name
    ...
    for col in columns_to_analyze:
        if col in schema.names:
            ...
            elif isinstance(schema[col].dataType, (TimestampType, DateType)): # Check for time types
                 datetime_cols.append(col)
            else:
                other_cols.append(col)
                logger.warning(f"列 '{col}' 类型为 {schema[col].dataType}，将从数值分析中排除。")
        else:
    ...
    try:
        ...
        agg_exprs = [F.count(F.col(c)).alias(f"{c}_count") for c in numerical_cols]
        agg_exprs.extend([F.mean(F.col(c)).alias(f"{c}_mean") for c in numerical_cols])
        agg_exprs.extend([F.stddev(F.col(c)).alias(f"{c}_stddev") for c in numerical_cols])
        agg_exprs.extend([F.min(F.col(c)).alias(f"{c}_min") for c in numerical_cols])
        agg_exprs.extend([F.expr(f"percentile_approx({c}, 0.25)").alias(f"{c}_25%") for c in numerical_cols])
        agg_exprs.extend([F.expr(f"percentile_approx({c}, 0.5)").alias(f"{c}_50%") for c in numerical_cols])
        agg_exprs.extend([F.expr(f"percentile_approx({c}, 0.75)").alias(f"{c}_75%") for c in numerical_cols])
        agg_exprs.extend([F.max(F.col(c)).alias(f"{c}_max") for c in numerical_cols])
    ...
    # 3. 检查负值 (Spark)
    precipitation_cols = ['precipitation', 'rain', 'snowfall', 'snow_depth', 'et0_fao_evapotranspiration', 'sunshine_duration', 'direct_radiation', 'diffuse_radiation'] # 这些理论上不应为负
...
def analyze_weather_categorical(sdf_weather: DataFrame, columns_to_analyze=None, top_n=20, plots_dir=None):
    ...
    for col in relevant_cols:
        ...
        try:
            ...
            counts_list = counts_sdf.take(top_n + 5) # Take more than top_n to see if there's a long tail
            if not counts_list:
            ...
            log_value_counts(counts_pd_renamed, col, top_n=top_n, is_already_counts=True) # top_n controls logging limit
            ...
            if plot:
                ...
                counts_series_top_n = counts_pd.set_index(col)['count'].head(top_n)
...
def analyze_weather_timeseries_sample(sdf_weather: DataFrame, n_samples=5, plots_dir=None, random_state=42):
    ...
    然后分析时间戳频率。
    """
    logger.info(f"--- Starting Spark analysis of Weather time series frequency (sampling {n_samples} location_id) ---")
    if plots_dir is None: # Plots dir might not be needed if only analyzing frequency
        logger.warning("plots_dir not provided, frequency analysis results will only be logged.")
    required_cols = ['location_id', 'timestamp']
    if not all(col in sdf_weather.columns for col in required_cols):
         logger.error(f"Input Weather Spark DataFrame missing required columns ({', '.join(required_cols)}). Cannot analyze frequency.")
         return
    ...
    try:
        ...
        if n_samples <= 0:
            logger.warning(f"Requested sample size ({n_samples}) is not positive, skipping weather frequency analysis.")
            return

        actual_n_samples = min(n_samples, num_distinct_ids)
        if num_distinct_ids < n_samples:
            logger.warning(f"Total location_id count ({num_distinct_ids}) is less than requested sample size ({n_samples}), using all {num_distinct_ids} location_ids.")

        logger.info(f"Randomly sampling {actual_n_samples} location_ids from {num_distinct_ids:,} using Spark RDD takeSample...")
        sampled_ids_rows = all_location_ids_sdf.rdd.takeSample(False, actual_n_samples, seed=random_state)
        ...
        sampled_ids_sdf = sdf_weather.sql_ctx.createDataFrame([(id,) for id in sampled_ids], ['location_id'])
        sdf_sample_filtered = sdf_weather.join(F.broadcast(sampled_ids_sdf), on='location_id', how='inner')
        ...
        try:
            ...
            if pdf_sample is None or pdf_sample.empty:
                ...
                return
            logger.info(f"Weather Pandas DataFrame created from Spark sample, containing {len(pdf_sample):,} rows for {len(sampled_ids)} location_ids.")
        ...
        logger.info("Sorting Weather Pandas DataFrame by location_id and timestamp...")
        pdf_sample = pdf_sample.sort_values(['location_id', 'timestamp'])
        if not pd.api.types.is_datetime64_any_dtype(pdf_sample['timestamp']):
             logger.info("Converting timestamp column to datetime objects in Pandas...")
             pdf_sample['timestamp'] = pd.to_datetime(pdf_sample['timestamp'], errors='coerce')
             if pdf_sample['timestamp'].isnull().any():
                 logger.warning("Some timestamp values failed to convert to datetime in Pandas for weather.")
                 pdf_sample = pdf_sample.dropna(subset=['timestamp'])

        # 5. 在 Pandas 上进行频率分析
        logger.info("Starting to analyze weather timestamp frequency for each sample (in Pandas)...")
        ...
        for location_id, df_id in tqdm(grouped_data, desc="Analyzing weather frequency (Pandas)"):
            if df_id.empty:
                logger.warning(f"No weather data found for location_id '{location_id}' in the Pandas sample, skipping frequency analysis.")
                continue
            ...
            try:
                 ...
                 if not time_diffs.empty:
                     ...
                     if not freq_counts.empty:
                         logger.info(f"--- location_id: {location_id} Weather Timestamp Interval Frequency (Pandas Sample) ---")
                         log_str = f"Frequency Stats (Top 5):\n{freq_counts.head().to_string()}"
                         if len(freq_counts) > 5: log_str += "\n..."
                         logger.info(log_str)
                         if len(freq_counts) > 1:
                             logger.warning(f" location_id '{location_id}' has multiple time intervals detected in weather sample.")
                     else:
                         logger.info(f" location_id '{location_id}' has only one unique time interval after diff/dropna in weather sample.")
                 else:
                     logger.info(f" location_id '{location_id}' has less than two timestamps in weather sample, cannot calculate intervals.")
            except Exception as freq_e:
                logger.exception(f"Error analyzing weather frequency for location_id '{location_id}' in sample: {freq_e}")

        logger.info("Weather time series frequency sample analysis (Spark filter -> Pandas analyze) complete.")

    except Exception as e:
        logger.exception(f"Error analyzing Weather time series frequency samples with Spark: {e}") 