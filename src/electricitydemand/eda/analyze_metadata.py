...
def analyze_metadata_categorical(pdf_metadata: pd.DataFrame, columns_to_analyze=None):
    ...
    for col in columns_to_analyze:
        ...
        if pdf_metadata[col].isnull().all():
            ...
            continue
        log_value_counts(pdf_metadata[col], col, top_n=20) # Use helper function, limit log output
...
def analyze_metadata_numerical(pdf_metadata: pd.DataFrame, columns_to_analyze=None, plots_dir=None):
    ...
    for col in columns_to_analyze:
        ...
        pdf_metadata_col_numeric = pd.to_numeric(pdf_metadata[col], errors='coerce')
        ...
        try:
            ...
            if original_missing != missing_count:
                 logger.warning(f"列 '{col}' 包含 {original_missing - missing_count} 个非数值类型的值被转换为 NaN。")

            if missing_count > 0:
                missing_perc = (missing_count / len(pdf_metadata) * 100).round(2)
                logger.warning(f"列 '{col}' 存在 {missing_count} ({missing_perc}%) 个缺失或无法转换的值。")
            ...
            if plot:
                ...
                if data_to_plot.empty:
                    ...
                    plot_numerical_distribution(data_to_plot, col,
                                                ...
                                                showfliers=(col != 'cluster_size')) # Maybe hide outliers for cluster_size
                 ...
                 if col in missing_df.columns:
                     ...
                     if not missing_df[col].isnull().all():
                         log_value_counts(missing_df[col], f"(缺失位置) {col}", top_n=None) # is_already_counts defaults to False
                          ...
                          logger.warning(f"列 '{col}' 在缺失位置信息的子集中为空或全为 null，跳过计数。")
