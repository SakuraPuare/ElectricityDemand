...
def log_basic_info(ddf_demand, ddf_metadata, ddf_weather):
    ...
    # 分区数 (可能因操作而变化) 和列名
    try:
        logger.info(f"Demand Spark DataFrame partitions: {ddf_demand.rdd.getNumPartitions()}, columns: {ddf_demand.columns}")
        logger.info(f"Metadata Spark DataFrame partitions: {ddf_metadata.rdd.getNumPartitions()}, columns: {ddf_metadata.columns}")
        logger.info(f"Weather Spark DataFrame partitions: {ddf_weather.rdd.getNumPartitions()}, columns: {ddf_weather.columns}")
    except Exception as e:
...
def check_missing_values(ddf_demand, ddf_metadata, ddf_weather, num_demand_rows, num_metadata_rows, num_weather_rows):
    ...
    def calculate_missing(df, total_rows, df_name):
        ...
        for field in schema.fields:
            ...
            is_numeric_for_nan = isinstance(col_type, (FloatType, DoubleType)) # 需要从 pyspark.sql.types 导入
            ...
            missing_exprs.append(
                F.sum(F.when(condition, 1).otherwise(0)).alias(f"{col_name}_missing_count")
            )

        missing_counts_df = df.agg(*missing_exprs).first() # .first() 触发计算并返回 Row 对象
        ...
        if missing_counts_df:
            ...
            for c in df.columns:
                ...
                if count_col_name in missing_counts_df.asDict():
                    ...
                    logger.warning(f"列 '{c}' 的缺失计数列 '{count_col_name}' 未在聚合结果中找到。")
            logger.info("\n" + "\n".join(missing_info))
        else:
...
def check_duplicates(ddf_demand, ddf_metadata, ddf_weather):
    ...
    def check_df_duplicates(df, key_columns, df_name):
        ...
        duplicate_counts = df.groupBy(key_columns).count().where(F.col('count') > 1)
        ...
        if num_duplicate_groups > 0:
            try:
                ...
                total_duplicate_rows_result = duplicate_counts.agg(F.sum('count')).first()
                if total_duplicate_rows_result and total_duplicate_rows_result[0] is not None:
                     total_duplicate_rows = total_duplicate_rows_result[0] # 计算重复组的总行数
                     logger.warning(f"{df_name} 数据中发现 {num_duplicate_groups} 组基于 {key_columns} 的重复记录。")
                     ...
                     logger.info(f"{df_name} 数据中未发现基于 {key_columns} 的重复记录 (聚合结果为空)。")
            except Exception as e:
    ...
    if 'unique_id' in ddf_demand.columns and 'timestamp' in ddf_demand.columns:
        ...
        logger.warning("Demand DataFrame 缺少 'unique_id' 或 'timestamp' 列，跳过重复检查。")
    ...
    if 'location_id' in ddf_weather.columns and 'timestamp' in ddf_weather.columns:
        check_df_duplicates(ddf_weather, ['location_id', 'timestamp'], "Weather")
    else:
        logger.warning("Weather DataFrame 缺少 'location_id' 或 'timestamp' 列，跳过重复检查。")
...
def log_time_ranges(ddf_demand, ddf_weather):
    ...
    try:
        if 'timestamp' in ddf_demand.columns:
            ...
            if demand_range:
                 logger.info(f"Demand 时间范围：从 {demand_range['min_ts']} 到 {demand_range['max_ts']}")
            else:
    ...
    try:
        if 'timestamp' in ddf_weather.columns:
            ...
            if weather_range:
                logger.info(f"Weather 时间范围：从 {weather_range['min_ts']} 到 {weather_range['max_ts']}")
            else:
...
def main():
    ...
    try:
        ...
        logger.info(f"Spark Web UI: {spark.sparkContext.uiWebUrl}") # 打印 Spark UI 地址
