    ...
    try:
        ...
        try:
            ...
            # --- 新增/修改检查 ---
                 ...
                 logger.warning("Weather 'timestamp' is TimestampNTZType, converting to TimestampType...")
                 sdf_weather = sdf_weather.withColumn("timestamp", F.col("timestamp").cast("timestamp"))
                 logger.info("Weather 'timestamp' converted to TimestampType.")
            elif "TimestampType" not in str(sdf_weather.schema["timestamp"].dataType):
                 logger.error(f"Weather 'timestamp' is of unexpected type: {sdf_weather.schema['timestamp'].dataType}. Join may fail.")
        ...
        try:
            ...
            sdf_merged_meta = sdf_demand.join(sdf_meta.alias("meta"), "unique_id", "left")
            logger.info("Demand 和 Metadata 合并完成。")
            # 使用 persist 代替 cache，更明确控制存储级别，这里默认 MEMORY_AND_DISK
            sdf_merged_meta.persist(StorageLevel.MEMORY_AND_DISK) # Cache the result as it's used multiple times below
            ...
            if weather_original_count != weather_dedup_count:
                ...
                logger.info(f"Weather 数据中无重复的 (location_id, timestamp) 记录。行数: {weather_dedup_count:,}")
            sdf_weather_dedup.persist(StorageLevel.MEMORY_AND_DISK) # Cache deduped weather data

            # --- Diagnostic Logging: 分析 location_id 覆盖情况 ---
            logger.info("--- [诊断] 开始分析 location_id 覆盖情况 ---")
            # 提取 Demand+Meta 中的 location_id (去重)
            demand_meta_locations = sdf_merged_meta.select("location_id").distinct()
            demand_meta_locations.persist()
            num_demand_meta_locations = demand_meta_locations.count()
            logger.info(f"[诊断] Demand+Meta 中唯一的 location_id 数量: {num_demand_meta_locations:,}")

            # 提取 Weather 中的 location_id (去重)
            weather_locations = sdf_weather_dedup.select("location_id").distinct()
            weather_locations.persist()
            num_weather_locations = weather_locations.count()
            logger.info(f"[诊断] 去重后 Weather 中唯一的 location_id 数量: {num_weather_locations:,}")

            # 计算交集 (有多少 location_id 在两者中都存在)
            common_locations_count = demand_meta_locations.join(weather_locations, "location_id", "inner").count()
            logger.info(f"[诊断] Demand+Meta 与 Weather 共有的 location_id 数量: {common_locations_count:,}")

            # 计算只在 Demand+Meta 中存在的 location_id (这些肯定无法匹配天气)
            demand_only_locations_count = demand_meta_locations.join(weather_locations, "location_id", "left_anti").count()
            logger.warning(f"[诊断] 只存在于 Demand+Meta 中的 location_id 数量 (无法匹配天气): {demand_only_locations_count:,}")
            if demand_only_locations_count > 0 and num_demand_meta_locations > 0:
                 percentage_missing_loc = (demand_only_locations_count / num_demand_meta_locations) * 100
                 logger.warning(f"[诊断] 这占 Demand+Meta 总 location_id 的 {percentage_missing_loc:.2f}%")
            ...
            logger.info("--- [诊断] 开始分析 Timestamp 范围对齐情况 (按 location_id, 基于原始时间戳) ---")
            try:
                ...
                ).orderBy(F.col("location_id").asc_nulls_last()) # Order for consistent logging, handle nulls
                ...
                for row in comparison_results:
                    ...
                    if loc_id is None:
                        ...
                        if min_w is not None or max_w is not None:
                            log_msg_parts.append(f" Weather({min_w} to {max_w})") # Should not happen if only demand has null loc_id
                            issues.append("Unexpected weather data for NULL location_id")
                        else:
                            log_msg_parts.append(" Weather(N/A)")
                        log_msg_parts.append(f" -> ISSUES: {'; '.join(issues)}")
                    ...
                    if min_d is None or max_d is None:
                         ...
                         issues.append("Demand data missing for this location_id in range analysis (unexpected)")
                    else:
                         log_msg_parts.append(f" Demand({min_d} to {max_d})")
                    ...
                    if min_w is None or max_w is None:
                         ...
                         if loc_id is not None: # Already checked for None
                            ...
                            issues.append("Weather data missing for this location_id")
                    else:
                    ...
                    if min_d and min_w and min_d < min_w:
                        ...
                        if time_diff.total_seconds() > 3600: # More than 1 hour diff
                            issues.append(f"Demand starts significantly EARLIER ({min_d} vs {min_w}, diff: {time_diff})")
                        # else: issue is minor or expected due to rounding
                    if max_d and max_w and max_d > max_w:
                        time_diff = max_d - max_w
                        if time_diff.total_seconds() > 3600: # More than 1 hour diff
                             issues.append(f"Demand ends significantly LATER ({max_d} vs {max_w}, diff: {time_diff})")
                ...
                if null_location_issues:
                    logger.error("[诊断] 检测到 NULL location_id，这通常是数据问题，可能导致部分数据无法 Join 天气。")

                if not mismatches_found and not null_location_issues:
                    logger.success("[诊断] 所有检查的 location_id 的时间戳范围看起来基本对齐或符合预期 (允许细微差异或无天气数据)。")
                elif not mismatches_found and null_location_issues:
                     logger.warning("[诊断] 时间戳范围基本对齐，但存在 NULL location_id 的问题。")
                else:
                    logger.warning("[诊断] 发现部分 location_id 的时间戳范围存在不匹配或缺失天气数据，并可能存在 NULL location_id。")

            except Exception as ts_diag_e:
                 logger.error(f"[诊断] 分析 Timestamp 范围时出错: {ts_diag_e}")
                 logger.exception("Traceback:") # Log traceback for timestamp errors
            logger.info("--- [诊断] Timestamp 范围对齐情况分析完毕 ---")
            # --- End Timestamp Diagnostic ---


            # 2.3 合并上一步结果和 Weather (保留所有 Demand 记录)
            logger.info("合并结果与 Weather (left join on location_id and timestamp_hour)...")
            ...
            sdf_final_merged = sdf_merged_meta.join(
                ...
                (sdf_merged_meta["timestamp_join_key"] == sdf_weather_aliased["timestamp"]),
                how="left"
            ...
            sdf_valid_locations = sdf_final_merged.where(F.col("location_id").isNotNull())
            valid_location_count = sdf_valid_locations.count()
            logger.info(f"[诊断] 其中 location_id 非 Null 的行数: {valid_location_count:,}")
            ...
            if key_weather_col in sdf_final_merged.columns:
                ...
                null_weather_count_valid_loc = sdf_valid_locations.where(F.col(key_weather_col).isNull()).count()

                logger.info(f"[诊断] 在 location_id 非 Null 的行中，'{key_weather_col}' 为 Null 的行数: {null_weather_count_valid_loc:,}")
                if valid_location_count > 0:
                    null_percentage = (null_weather_count_valid_loc / valid_location_count) * 100
                    # 预期这个比例会非常低，接近 0%
                    log_level = logger.warning if null_percentage > 1.0 else logger.info # Log as warning only if > 1% nulls remain
                    log_level(f"[诊断] 天气数据 Join 成功率 (基于 '{key_weather_col}', 排除 Null location_id): {100.0 - null_percentage:.2f}% (缺失率: {null_percentage:.2f}%)")
                else:
                    logger.info("[诊断] 没有 location_id 非 Null 的行，无法计算有效 Join 成功率。")

                # Also log total null count for reference
                total_null_weather_count = sdf_final_merged.where(F.col(key_weather_col).isNull()).count()
                logger.info(f"[诊断] 最终合并数据中 '{key_weather_col}' 总 Null 行数 (包括 Null location_id): {total_null_weather_count:,}")
                if final_merged_count > 0:
                     total_null_perc = (total_null_weather_count / final_merged_count) * 100
                     logger.info(f"[诊断] 占总行数的比例: {total_null_perc:.2f}%")


            else:
                 logger.error(f"[诊断] 关键天气列 '{key_weather_col}' 不在最终合并结果中！无法检查 Null 比例。")
            ...
            # 清理缓存
            if sdf_merged_meta.is_cached: sdf_merged_meta.unpersist()
            if sdf_weather_dedup.is_cached: sdf_weather_dedup.unpersist()


        except Exception as merge_e:
            logger.exception(f"数据合并过程中出错: {merge_e}")
            if 'sdf_merged_meta' in locals() and sdf_merged_meta.is_cached: sdf_merged_meta.unpersist() # Clean up cache on error
            if 'sdf_weather_dedup' in locals() and sdf_weather_dedup.is_cached: sdf_weather_dedup.unpersist()
            raise
