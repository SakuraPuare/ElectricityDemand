...
def plot_numerical_distribution(data_pd_series, col_name, filename_base, plots_dir, title_prefix="", kde=True, showfliers=True):
    ...
    try:
        ...
        save_plot(fig, f"{filename_base}.png", plots_dir)
    except Exception as e:
        logger.exception(
...
def plot_categorical_distribution(data_pd_series, col_name, filename_base, plots_dir, top_n=10, title_prefix=""):
    ...
    try:
        ...
        # 如果输入是原始 Series
            ...
            value_counts = data_pd_series.value_counts(
        ...
        try:
            ...
            logger.debug(
                f"Cannot sort index for '{col_name}' easily, using count order.")
            sort_order = data_to_plot.index.astype(
                str)  # Order by appearance (count order)
