...
def _resample_group_pd(pdf: pd.DataFrame) -> pd.DataFrame:
    ...
    try:
        ...
        logger.error(f"Error resampling group for unique_id {pdf['unique_id'].iloc[0] if not pdf.empty else 'unknown'}: {e}")
...
def resample_demand_to_hourly(ddf_demand: dd.DataFrame) -> dd.DataFrame | None:
    ...
    if not required_cols.issubset(ddf_demand.columns):
        logger.error(f"Input DataFrame missing required columns (need {required_cols}, have {ddf_demand.columns}). Cannot resample.")
        return None
    ...
    try:
        ...
        if not pd.api.types.is_datetime64_any_dtype(ddf_demand['timestamp'].dtype):
             logger.warning("Demand 'timestamp' column is not datetime type. Attempting conversion...")
             ddf_demand = ddf_demand.map_partitions(
                 lambda pdf: pdf.assign(timestamp=pd.to_datetime(pdf['timestamp'], errors='coerce')),
                 meta={'unique_id': 'str', 'timestamp': 'datetime64[ns]', 'y': 'float64'}
        ...
        if isinstance(resampled_ddf.index, pd.MultiIndex): # Check if it's actually MultiIndex as expected
             resampled_ddf = resampled_ddf.reset_index()
             logger.info("重置 MultiIndex 完成。")
        else:
             logger.warning(f"预期得到 MultiIndex，但实际得到：{type(resampled_ddf.index)}. 尝试 reset_index()...")
             try:
                 resampled_ddf = resampled_ddf.reset_index()
        ...
        if not required_cols.issubset(resampled_ddf.columns):
             logger.error(f"重置索引后仍然缺少必需列。需要：{required_cols}, 实际：{resampled_ddf.columns}")
             logger.info("重采样后的数据结构预览 (meta):")
             logger.info(f"\n{resampled_ddf._meta.head()}")
             return None
        ...
        # 打印一些信息以供验证
        logger.info("重采样后的数据结构预览 (meta):")
        logger.info(f"\n{resampled_ddf._meta.head()}") # Log the meta data head
...
def validate_resampling(ddf_resampled: dd.DataFrame | None, n_check: int = 5):
    ...
    try:
        ...
        if not required_cols.issubset(ddf_resampled.columns):
             logger.error(f"验证失败：重采样后的 DataFrame 缺少必需列。需要：{required_cols}, 实际：{ddf_resampled.columns}")
             logger.info("重采样后的数据结构预览 (meta):")
        ...
        sample_df_pd = ddf_resampled[['timestamp', 'unique_id']].head(n_check, compute=True)
        if sample_df_pd.empty:
            logger.warning("无法获取样本时间戳进行验证 (可能数据为空？)")
            return

        all_hourly = all((ts.minute == 0 and ts.second == 0) for ts in sample_df_pd['timestamp'])
