...
try:
    ...
    from electricitydemand.eda.load_data import load_datasets # Use the correct path if it moved
    ...
    from electricitydemand.eda.analyze_relationships import (
        ...
        analyze_demand_vs_weather, # 保留这个
    )
...
def run_all_eda():
    ...
    try:
        ...
        logger.info(f"系统总内存: {total_memory_gb:.2f}GB, 设置驱动器内存: {driver_memory}g, 执行器内存: {executor_memory}g")
        ...
        if pdf_metadata is not None:
             ...
             raise ValueError("Failed to convert Metadata to Pandas.") # Stop execution if metadata is needed
        ...
        if pdf_metadata is not None: # Check again, though we raise error if it fails now
            ...
            logger.info("--- 开始 Demand vs Weather 分析 (Spark Join -> Pandas Collect) ---")
            analyze_demand_vs_weather(sdf_demand, pdf_metadata, sdf_weather, spark, plots_dir, n_sample_ids=50)
        ...
        logger.info(f"--- EDA 脚本总执行时间: {end_run_time - start_run_time:.2f} 秒 ---")
